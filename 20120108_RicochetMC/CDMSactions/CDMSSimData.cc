// $Id: CDMSSimData.cc,v 1.4 2011/07/05 21:22:24 kelsey Exp $
////////////////////////////////////////////////////////////////////////
//                                                                    //
//  File:        CDMSSimData.cc                                       //
...
//  20110425  M. Kelsey -- Remove extraneous tabs from column heading //
...
CDMSSimData::CDMSSimData(G4int verbose)
  : verboseLevel(verbose), outputFile(0) {}

CDMSSimData::~CDMSSimData()
{
  WriteAndClose();	// Just in case the user forgot
}


void CDMSSimData::SetupFile(const G4String& prefix, G4int runNumber,
			    G4int labCode) {
  ...
  if (verboseLevel)
    G4cout << " Creating CDMS simulation data file " << fileName << G4endl;
...
G4String CDMSSimData::FileName(const G4String& prefix, G4int runNumber,
			       G4int labCode) const {
...
void CDMSSimData::ProcessHits(const G4Event* evt, CDMSZipHitsCollection* ZHC)
{
...
void CDMSSimData::ProcessHits(const G4Event* evt, CDMSVetoHitsCollection* VHC)
{
...
void CDMSSimData::LoadHitData(const G4Event* evt, const CDMSZipHit* ZipHit)
{
...
void CDMSSimData::LoadHitData(const G4Event* evt, const CDMSVetoHit* VetoHit) {
...
void CDMSSimData::WriteAndClose() {
