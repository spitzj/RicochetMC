// $Id: CDMSSimRoot.cc,v 1.10 2011/07/05 21:22:24 kelsey Exp $
////////////////////////////////////////////////////////////////////////
//                                                                    //
//  File:        CDMSSimRoot.cc                                       //
...
//  20110705  M. Kelsey -- Fix argument order to FileName call, use   //
//		 tm_mon+1 for filename string.                        //
////////////////////////////////////////////////////////////////////////
...
CDMSSimRoot::CDMSSimRoot(G4int verbose)
  : verboseLevel(verbose), tfile(0), eventTree(0), vetoTree(0) {
  for (G4int i = 0; i < CDMSSimRoot::NZIPS; i++) zipTree[i] = 0;
}

CDMSSimRoot::~CDMSSimRoot()
{
  WriteAndClose();	// Just in case the user forgot
}


void CDMSSimRoot::ProcessEventInfo(const G4Event* evt)
{
...
void CDMSSimRoot::ProcessHits(const G4Event* evt, CDMSZipHitsCollection* ZHC)
{
...
void CDMSSimRoot::ProcessHits(const G4Event* evt, CDMSVetoHitsCollection* VHC)
{
...
void CDMSSimRoot::LoadHitData(const G4Event* evt, const CDMSZipHit* ZipHit)
{
...
void CDMSSimRoot::LoadHitData(const G4Event* evt, const CDMSVetoHit* VetoHit) {
...
void CDMSSimRoot::LoadEventData(const G4Event* evt)
{
  if (!evt) return;
...
void CDMSSimRoot::WriteAndClose()
{
  if (verboseLevel) G4cout << ">>> CDMSSimRoot::WriteAndClose" << G4endl;

  for (G4int i = 0; i < CDMSSimRoot::NZIPS; i++) {
    if (zipTree[i]) {
      zipTree[i]->Write();
      delete zipTree[i];
      zipTree[i] = 0;
...
void CDMSSimRoot::SetupFile(const G4String& prefix, G4int runNumber,
			    G4int labCode) {
  ...
  if (verboseLevel)
    G4cout << " Creating CDMS simulation ROOT file " << fileName << G4endl;
...
G4String CDMSSimRoot::FileName(const G4String& prefix, G4int runNumber,
			       G4int labCode) const {
...
void CDMSSimRoot::CreateEventTree()
{
...
void CDMSSimRoot::CreateVetoTree()
{ 
...
void CDMSSimRoot::CreateAZipTree(G4int i)
{
