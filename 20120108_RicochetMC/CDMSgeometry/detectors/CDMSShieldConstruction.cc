// $Id: CDMSShieldConstruction.cc,v 1.30 2011/07/22 21:09:17 kelsey Exp $
////////////////////////////////////////////////////////////////////////
//  File:        CDMSShieldConstruction.cc                            //
//                                                                    //
//  Description: CDMS shield and veto arrangement outside of icebox   //
//                                                                    //
//  Author:      Dennis Wright (SLAC)                                 //
//  Date:        3 December 2010                                      //
//                                                                    //
//  20110105  M. Kelsey -- Add and use pointer to cryostat; replace   //
//		local "eps" with base class "tolerance".   Messenger. //
//  20110105  M. Kelsey -- Follow icebox change to PipeBaseLen.       //
//  20110106  M. Kelsey -- Simplify parameter usage with structs, use //
//		box for mother volume (to enclose top and bottom).    //
//  20110107  M. Kelsey -- Move veto parameters to data members, set  //
//		in FillExtraParameters(), report in PrintParameters().//
//		New function to make pipe penetrations.               //
//  20110110  M. Kelsey -- Pipe hole function needs full three-vector //
// 		Add parameters for number of veto panels              //
//  20110112  M. Kelsey -- Replace shiftFactor with corner overlap,   //
//		use trig functions to accurately compute shifts.      //
//  20110113  M. Kelsey -- Check for even-sided polygon veto.         //
//  20110114  M. Kelsey -- Handle pipe holes in arbitrary veto panels //
//  20110211  M. Kelsey -- Allow user to suppress veto panels w/N=0;  //
//		fix bug in sizing of inner subtracted layer blocks    //
//  20110215  M. Kelsey -- Report mass at end of construction.        //
//  20110216  M. Kelsey -- Add reports of mass for each shield layer. //
//  20110328  M. Kelsey -- Fetch cryostat using Manager.              //
...
CDMSShieldConstruction::LayerData::
...
CDMSShieldConstruction::LayerData::~LayerData() {
...
void 
CDMSShieldConstruction::LayerData::FillOuterShield(G4double& radius, G4double& length,
					G4double& z) {
...
void 
CDMSShieldConstruction::LayerData::FillInnerShield(G4double& radius, G4double& length,
					G4double& z) {
  Radius = (radius -= (Gap + SideThick));
  Length = (length -= (2.*Gap + TopThick + BottomThick));
  Mid    = (BottomThick - TopThick)/2.;
...
CDMSShieldConstruction::CDMSShieldConstruction()
  : CDMSVDetectorGeometry("VetoShield"), icebox(new CDMSVesselConstruction()), DetectorName("Veto"),
    ...
    messenger(new CDMSShieldMessenger(this)) {}
...
void CDMSShieldConstruction::SetVerboseLevel(G4int verbose) {
...
void CDMSShieldConstruction::UseIcebox() {
  icebox = CDMSGeometryManager::Instance()->GetVessel();
}


// Check here for valid polygons, report error
void CDMSShieldConstruction::SetNSidePanels(G4int value) {
  if (value > 0 && value < 4) {
    value = 3;
    G4cerr << " CDMSShieldConstruction: side panels must form a polygon."
	   << "  N set to 3" << G4endl;
  }
...
void CDMSShieldConstruction::FillExtraParameters() {
  SetVerboseLevel(verboseLevel);	// Ensure that children have verbosity

  if (verboseLevel)
    G4cout << "CDMSShieldConstruction::FillExtraParameters()" << G4endl;
...
void CDMSShieldConstruction::FillIceboxParameters() {
  if (!icebox) return;

  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::FillIceboxParameters()" << G4endl;
...
void CDMSShieldConstruction::FillShieldParameters() {
  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::FillShieldParameters()" << G4endl;
...
void CDMSShieldConstruction::FillVetoParameters() {
  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::FillVetoParameters()" << G4endl;

  FillEndcapVetoParameters();
  FillBarrelVetoParameters();
}

void CDMSShieldConstruction::FillEndcapVetoParameters() {
  NTopPanels = NTopPanelsX*NTopPanelsY;
  if (0 == NTopPanels) return;			// User suppressed panels

  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::FillEndcapVetoParameters()" << G4endl;
...
void CDMSShieldConstruction::FillBarrelVetoParameters() {
  if (0 == NSidePanels) return;		// User suppressed panels

  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::FillBarrelVetoParameters()" << G4endl;
...
void CDMSShieldConstruction::FillMotherParameters() {
  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::FillMotherParameters()" << G4endl;
...
G4LogicalVolume* CDMSShieldConstruction::BuildGeometry() {
  if (verboseLevel)
    G4cout << "CDMSShieldConstruction::BuildGeometry()" << G4endl;
...
G4LogicalVolume* CDMSShieldConstruction::CreateMotherVolume() {
  if (verboseLevel)
    G4cout << "CDMSShieldConstruction::CreateMotherVolume()" << G4endl;
  ...
  G4LogicalVolume* logicalShieldVolume = 
    new G4LogicalVolume(box, CDMSMaterialTable::GetMaterial(mother.matName),
                        mother.Name, 0, 0, 0);
...
void CDMSShieldConstruction::PlaceShielding(G4LogicalVolume* mother)
{
  if (verboseLevel)
    G4cout << "CDMSShieldConstruction::PlaceShielding()" << G4endl;
...
void CDMSShieldConstruction::PlaceShieldLayer(const LayerData& layer,
					      G4LogicalVolume* mother, G4bool innerType) {
  if (verboseLevel > 1)
    G4cout << "CDMSShieldConstruction::PlaceShieldLayer " << layer.Name
	   << G4endl;
...
G4LogicalVolume*
CDMSShieldConstruction::BuildShieldLayer(const LayerData& layer, G4bool innerType) {
  if (verboseLevel > 1)
    G4cout << "CDMSShieldConstruction::BuildShieldLayer " << layer.Name
	   << G4endl;
  ...
  G4LogicalVolume* logicalShieldLayer =
    new G4LogicalVolume(shell, CDMSMaterialTable::GetMaterial(layer.matName),
...
G4VSolid*
CDMSShieldConstruction::MakePipeHoleInSolid(G4VSolid* solid,
					    G4double radius, G4double thick,
					    const G4ThreeVector& hole) {
  if (verboseLevel > 1) {
    G4cout << "CDMSShieldConstruction::MakePipeHoleInSolid"
...
void CDMSShieldConstruction::PlaceVeto(G4LogicalVolume* mother)
{
  if (verboseLevel) G4cout << "CDMSShieldConstruction::PlaceVeto()" << G4endl;
...
void CDMSShieldConstruction::PlaceEndcapVeto(G4LogicalVolume* mother)
{
  if (0 == NTopPanels) return;		// User suppressed veto

  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::PlaceEndcapVeto()" << G4endl;

  G4Material* scint = CDMSMaterialTable::GetMaterial("G4_PLASTIC_SC_VINYLTOLUENE");
...
void CDMSShieldConstruction::PlaceBarrelVeto(G4LogicalVolume* mother)
{
  if (0 == NSidePanels) return;		// User suppressed veto

  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::PlaceBarrelVeto()" << G4endl;
...
void CDMSShieldConstruction::PlaceTopSideVeto(G4LogicalVolume* mother)
{
  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::PlaceTopSideVeto()" << G4endl;
...
void CDMSShieldConstruction::PlaceMiddleSideVeto(G4LogicalVolume* mother)
{
  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::PlaceMiddleSideVeto()" << G4endl;
...
void CDMSShieldConstruction::PlaceBottomSideVeto(G4LogicalVolume* mother)
{
  if (verboseLevel>1)
    G4cout << "CDMSShieldConstruction::PlaceBottomSideVeto()" << G4endl;
...
void CDMSShieldConstruction::GetCornersOfPanel(G4int index, G4Point3D& right,
					       G4Point3D& left) {
  ...
  if (verboseLevel > 2) {
    G4cout << "CDMSShieldConstruction::GetCornersOfPanel " << index
	   << "\n right = " << right << " left = " << left << G4endl;
  }
}

G4bool CDMSShieldConstruction::PanelIntersectsVacStem(G4int index,
						      G4double& panelY) {
...
G4bool CDMSShieldConstruction::PanelIntersectsFridgeStem(G4int index,
							 G4double& panelY) {
  G4Point3D rightCorner, leftCorner;
  GetCornersOfPanel(index, rightCorner, leftCorner);
...
G4double CDMSShieldConstruction::DistanceToPipe(const G4Point3D& right,
						const G4Point3D& left) {
  if (verboseLevel > 2) 
    G4cout << "CDMSShieldConstruction::DistanceToPipe" << G4endl;
...
G4LogicalVolume* 
CDMSShieldConstruction::MakeVacHoleInPanel(G4VSolid* panel, G4int index,
					   G4double holeY) {
  if (verboseLevel > 1)
    G4cout << "CDMSShieldConstruction::MakeVacHoleInPanel " << index << G4endl;

  return MakePipeHoleInPanel(panel, index, vacStemRad, holeY, zVacStem);
}

G4LogicalVolume* 
CDMSShieldConstruction::MakeFridgeHoleInPanel(G4VSolid* panel, G4int index,
					      G4double holeY) {
  if (verboseLevel > 1)
    G4cout << "CDMSShieldConstruction::MakeFridgeHoleInPanel " << index
	   << G4endl;
...
G4LogicalVolume* 
CDMSShieldConstruction::MakePipeHoleInPanel(G4VSolid* panel, G4int index,
					    G4double radius, G4double holeY,
					    G4double holeZ) {
  if (verboseLevel > 2) {
    G4cout << "CDMSShieldConstruction::MakePipeHoleInPanel " << index 
...
G4VSensitiveDetector* CDMSShieldConstruction::BuildSensitiveDetector()
{
  ...
  if (!vetoSD) {
    ...
    vetoSD = new CDMSVetoSD(DetectorName);
...
void CDMSShieldConstruction::PrintParameters(std::ostream& os) const {
  os << "CDMSShieldConstruction parameters"
...
std::ostream& 
operator<<(std::ostream& os, const CDMSShieldConstruction::LayerData& layer) {
